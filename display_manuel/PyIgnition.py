...
class ParticleEffect:
	...
	def CreateSource(self, pos = (0, 0), initspeed = 0.0, initdirection = 0.0, initspeedrandrange = 0.0, initdirectionrandrange = 0.0, particlesperframe = 0, particlelife = -1, genspacing = 0, drawtype = 0, colour = (0, 0, 0, 0), radius = 0.0, length = 0.0, imagepath = None):
	...
	def CreateCircle(self, pos = (0, 0), colour = (0, 0, 0, 0), bounce = 1.0, radius = 0.0):
		newcircle = obstacles.Circle(self, pos, colour, bounce, radius)
		self.obstacles.append(newcircle)
		return newcircle
	
	def CreateRectangle(self, pos = (0, 0), colour = (0, 0, 0, 0), bounce = 1.0, width = 0.0, height = 0.0):
		newrect = obstacles.Rectangle(self, pos, colour, bounce, width, height)
		self.obstacles.append(newrect)
		return newrect
	
	def CreateBoundaryLine(self, pos = (0, 0), colour = (0, 0, 0, 0), bounce = 1.0, normal = [0, 1]):
	...
	def SaveToFile(self, outfilename):
		...
		for source in self.sources:
			...
			outfile.write("\t\t<colour>(%i, %i, %i, %i)</colour>\n" % source.colour)
			...
			if child.tag == "source":  # Source object
				...
				for keyframe in particlekeyframes:
					...
					for variable in keyframe.children:
						...
						elif variable.tag == "colour_a" and variable.inside != "None":
							variables['colour_a'] = int(variable.inside)
...
if __name__ == '__main__':
	...
	testsource.CreateParticleKeyframe(125, colour = (0, 0, 0, 0), length = 10.0)
