...
class Circle(Obstacle):
	...
	def Update(self):
		newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_b':self.colour[3], 'bounce':self.bounce, 'radius':self.radius}, self.keyframes)
		self.pos = (newvars['pos_x'], newvars['pos_y'])
		self.colour = (newvars['colour_r'], newvars['colour_g'], newvars['colour_b'], newvars['colour_a'])
	...
	def CreateKeyframe(self, frame, pos = (None, None), colour = (None, None, None), bounce = None, radius = None, interpolationtype = INTERPOLATIONTYPE_LINEAR):
		return keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x':pos[0], 'pos_y':pos[1], 'colour_r':colour[0], 'colour_g':colour[1], 'colour_b':colour[2], 'colour_a':colour[3], 'bounce':bounce, 'radius':radius, 'interpolationtype':interpolationtype})
	
	def ConsolidateKeyframes(self):
		keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_a':self.colour[3], 'bounce':self.bounce, 'radius':self.radius})
...
class Rectangle(Obstacle):
	...
	def Update(self):
		newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_a':self.colour[3], 'bounce':self.bounce, 'width':self.width, 'height':self.height}, self.keyframes)
		self.pos = (newvars['pos_x'], newvars['pos_y'])
		self.colour = (newvars['colour_r'], newvars['colour_g'], newvars['colour_b'], newvars['colour_a'])
		self.bounce = newvars['bounce']
	...
	def CreateKeyframe(self, frame, pos = (None, None), colour = (None, None, None), bounce = None, width = None, height = None, interpolationtype = INTERPOLATIONTYPE_LINEAR):
		return keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x':pos[0], 'pos_y':pos[1], 'colour_r':colour[0], 'colour_g':colour[1], 'colour_b':colour[2], 'colour_a':colour[3], 'bounce':bounce, 'width':width, 'height':height, 'interpolationtype':interpolationtype})
	
	def ConsolidateKeyframes(self):
		keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_a':self.colour[3], 'bounce':self.bounce, 'width':self.width, 'height':self.height})
...
class BoundaryLine(Obstacle):
	...
	def Update(self):
		newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_a':self.colour[3], 'bounce':self.bounce, 'normal_x':self.normal[0], 'normal_y':self.normal[1]}, self.keyframes)
		self.pos = (newvars['pos_x'], newvars['pos_y'])
		self.colour = (newvars['colour_r'], newvars['colour_g'], newvars['colour_b'], newvars['colour_a'])
		self.bounce = newvars['bounce']
	...
	def CreateKeyframe(self, frame, pos = (None, None), colour = (None, None, None, None), bounce = None, normal = [None, None], interpolationtype = INTERPOLATIONTYPE_LINEAR):
		if (normal != [None, None]) and (abs(magnitudesquared(normal) - 1.0) >= 0.3):
			normal = normalise(normal)
		return keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x':pos[0], 'pos_y':pos[1], 'colour_r':colour[0], 'colour_g':colour[1], 'colour_b':colour[2],'colour_a':colour[3], 'bounce':bounce, 'normal_x':normal[0], 'normal_y':normal[1], 'interpolationtype':interpolationtype})
	
	def ConsolidateKeyframes(self):
		keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {'pos_x':self.pos[0], 'pos_y':self.pos[1], 'colour_r':self.colour[0], 'colour_g':self.colour[1], 'colour_b':self.colour[2],'colour_b':self.colour[3], 'bounce':self.bounce, 'normal_x':self.normal[0], 'normal_y':self.normal[1]})
