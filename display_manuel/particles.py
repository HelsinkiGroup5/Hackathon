...
class Particle:
	def __init__(self, parent, initpos, velocity, life, drawtype = DRAWTYPE_POINT, colour = (0, 0, 0, 0), radius = 0.0, length = 0.0, image = None, keyframes = []):
		self.parent = parent
		self.pos = initpos
	...
	def Update(self):
		...
		if self.life != -1 and self.curframe > self.life:
			...
			if self.life == -1 and self.curframe >= len(self.parent.particlecache):  # If the particle has infinite life and has passed the last cached frame
				self.colour = (self.parent.particlecache[len(self.parent.particlecache) - 1]['colour_r'], self.parent.particlecache[len(self.parent.particlecache) - 1]['colour_g'], self.parent.particlecache[len(self.parent.particlecache) - 1]['colour_b'], self.parent.particlecache[len(self.parent.particlecache) - 1]['colour_a'])
				...
				self.colour = (self.parent.particlecache[self.curframe]['colour_r'], self.parent.particlecache[self.curframe]['colour_g'], self.parent.particlecache[self.curframe]['colour_b'], self.parent.particlecache[len(self.parent.particlecache) - 1]['colour_a'])
	...
	def Draw(self, display):
		...
		elif self.drawtype == DRAWTYPE_CIRCLE:
	...
	def CreateKeyframe(self, frame, colour = (None, None, None, None), radius = None, length = None):
		keyframes.CreateKeyframe(self.keyframes, frame, {'colour_r':colour[0], 'colour_g':colour[1], 'colour_b':colour[2], 'colour_a':colour[3], 'radius':radius, 'length':length})



class ParticleSource:
	def __init__(self, parenteffect, pos, initspeed, initdirection, initspeedrandrange, initdirectionrandrange, particlesperframe, particlelife, genspacing, drawtype = 0, colour = (0, 0, 0, 0), radius = 0.0, length = 0.0, imagepath = None):
	...
	def CreateParticleKeyframe(self, frame, colour = (None, None, None, None), radius = None, length = None, interpolationtype = INTERPOLATIONTYPE_LINEAR):
		newframe = keyframes.CreateKeyframe(self.particlekeyframes, frame, {'colour_r':colour[0], 'colour_g':colour[1], 'colour_b':colour[2],'colour_a':colour[3], 'radius':radius, 'length':length, 'interpolationtype':interpolationtype})
	...
	def PreCalculateParticles(self):
		...
		for i in xrange(0, particlelife + 1):
			vars = interpolate.InterpolateKeyframes(i, {'colour_r':0, 'colour_g':0, 'colour_b':0,'colour_a':0, 'radius':0, 'length':0}, self.particlekeyframes)
